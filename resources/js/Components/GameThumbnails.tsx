...
export default function GameThumbnail ({game}:{game: GameData}) {
    ...
    return (
        <Link href={"/" + game.slug} key={game.id}>
            {
                noThumbnail 
                    ? <div className="relative w-[322px] h-[152px] border border-black rounded-2xl m-1 group transition-all hover:scale-110 hover:shadow-[0/5px/5px/5px/black]" title={game.name}>
                        ...
                        <span className="absolute bottom-1 left-1 p-1 border rounded-md text-white border-black bg-black bg-opacity-80 transition-all group-hover:text-mainOrange">
                            {game.release_date ? replaceMonthWithName(game.release_date) : game.release_window}
                        </span>
                        {
                            game.early_access === true 
